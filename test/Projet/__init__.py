...
class Projet:
    ...
    def enregistrer_changement(self, description: str):
        self.changements.append(
            Changement(description, self.version, datetime.datetime.now())
        )
        self.version += 1
        self.notifier(
    ...
    def calculer_chemin_critique(self):
        ...
        for tache in self.taches:
            if tache.dependances:
                date_debut_tot[tache] = max(
                    date_fin_tot[dependance] for dependance in tache.dependances
                )
            date_fin_tot[tache] = date_debut_tot[tache] + (
        ...
        for tache in reversed(self.taches):
            if tache != tache_finale and tache.dependances:
                date_fin_tard[tache] = min(
                    date_debut_tard[dependance] for dependance in tache.dependances
                )
        ...
        self.chemin_critique = [
            tache
            for tache in self.taches
            if date_debut_tard[tache] == date_debut_tot[tache]
        ]
